...
class WindowsCompatibilityTest(unittest.TestCase):
    ...
    def test_windows_ci_release_and_pack_workflows_are_first_class(self) -> None:
        ...
        self.assertIn('$workspace = Join-Path $env:RUNNER_TEMP "Project Brain 原生 空格"', release)
        self.assertIn('$longSource = Join-Path $nativeRepo (("long-segment\\" * 15)', release)
        self.assertNotIn('("Project Brain 原生 空格 " + ("long-segment-" * 18))', release)
