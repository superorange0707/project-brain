...
class ReleaseSafetyTest(unittest.TestCase):
    ...
    def test_homebrew_formula_is_rendered_only_from_final_release_checksums(self) -> None:
        ...
        self.assertIn("brew install --formula \"$VERIFICATION_NAME\"", workflow)
        self.assertIn("brain --version", workflow)
        self.assertIn("brain --help >/dev/null", workflow)
        self.assertIn("brew test \"$VERIFICATION_NAME\"", workflow)
    ...
    def test_standalone_release_builds_source_pinned_zoekt(self) -> None:
        ...
        self.assertIn('expected_platform == "darwin-arm64"', workflow)
        self.assertIn('expected_fallback = "Local reranker failed; used semantic/lexical candidate ranking."', workflow)
        self.assertIn("reranker_elapsed_ms >= normal_timeout_ms", workflow)
        self.assertIn("installed conformance and explicit Semantic/lexical fallback both passed", workflow)
