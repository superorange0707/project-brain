...
```bash
tar -xzf project-brain-v1.0.7-macos-arm64.tar.gz
mkdir -p ~/.local/bin
install brain codebase-memory-mcp zoekt zoekt-index ~/.local/bin/
```

For a manual Windows installation, download
`project-brain-v1.0.7-windows-amd64.zip`, verify it against the published
...
After upgrading to v1.0.7, rerun `brain agent-kit m365`, replace Instructions
and `PROJECT_KNOWLEDGE.md`, and optionally refresh Suggested Prompts and the
protocol guide. `AGENT_KIT.json` identifies Brain 1.0.7, Agent Kit v4, and
