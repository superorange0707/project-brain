...
**Snapshot:** 2026-09-02 · **v1.0 implementation gate:** COMPLETE; Python 3.11–3.14 remains mandatory · **Clean baseline:** v0.9.2 (`b22cfee8058f0c9772a1372e53a0401efd2e7983`)
...
| v1.0 Investigation Runtime | COMPLETE | Generation-scoped runtime anchors and Java/Spring intelligence; ordered execution/integration flows; Program Slice Lite; implementation/test/impact/contract/config-data surfaces; Hypothesis Ledger and Evidence Frontier; bounded multi-wave controller; stable v5 identities and checkpoint lineage; M365 Agent Kit v4; Investigation Cockpit; integrated evaluation/scale/adversarial coverage; permanent Python 3.11–3.14 gates; and native Windows source/CI/release paths. The real Windows matrix, standalone, and official Semantic/Precision model-pack gates passed. |
...
- `v0.6.7` is the current published Core release. Its official Homebrew formula
  ...
  mutation.
- Project Brain v1.0.8 is implemented from exact v0.9.2 commit
  ...
  publication is authorized. The v1.0.8 release candidate additionally passed
  native macOS and GitHub-hosted Windows Semantic/Precision installation,
  conformance, generation-alignment, and retrieval readiness gates. Its final
  Windows release hardening keeps the long-path fixture below the Git process
  working-directory boundary and eliminates redundant scans of comment-masked
  Java tails without changing Atlas identity or bounded parsing contracts. The
  native CLI also establishes UTF-8 redirected streams before printing Unicode
  workspace paths. Its Windows UI release smoke tolerates the brief empty-log
  interval before the already-flushed startup token becomes visible, and the
  native cross-process lock gate uses a bounded startup allowance with retained
  child-process diagnostics for loaded hosted runners. Its macOS readiness gate
  keeps installed model conformance and serving-alignment requirements while
  recognizing the product's explicit Semantic/lexical fallback when a hosted
  runner exceeds the normal online reranker budget. The v1.0.8 patch removes
  repeated model reloads and per-batch cache scans/commits from bulk Semantic
